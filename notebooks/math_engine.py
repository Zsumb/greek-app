...
Importable from sibling scripts:
...
# === Single-option Black-Scholes pricer (European) ===
...
@dataclass
class Greeks:
    """Greeks (per share for single options, contract-scaled for positions)."""
    price: float
    delta: float                # $ per $1 move in S
    gamma: float                # delta change per $1 move
    theta_per_day: float        # $ per calendar day
    vega_per_volpoint: float    # $ per 1 vol-point (0.01 in sigma)
    rho_per_pct: float          # $ per 1% (0.01) in rates
...
class Leg:
    kind: OptionType
    strike: float
    expiry_days: int   # calendar days from position-open
    quantity: int      # contracts; negative = short


@dataclass
class Position:
    legs: list[Leg]
    S: float           # spot at position-open
    sigma: float       # IV at position-open (single value across legs for now)
    r: float

    def price(self, S: Optional[float] = None, sigma: Optional[float] = None,
              days_elapsed: int = 0) -> float:
        """Total position value (across legs, contract-scaled)."""
    ...
    def greeks(self, S: Optional[float] = None, sigma: Optional[float] = None,
        ...
        """Aggregated Greeks across legs (contract-scaled)."""
        S_use = S if S is not None else self.S
        sig_use = sigma if sigma is not None else self.sigma
...
def simulate(position: Position, dS: float, dSigma: float, dDays: int) -> SimulationResult:
    """
    Simulate `position` under a scenario shock.
        dS     : change in spot ($)
        dSigma : change in IV (decimal; -0.01 = -1 vol-point)
        dDays  : calendar days forward
    Returns actual P&L, the four-Greek decomposition, and the residual.
    """
